...
{
    v.insert(v.end(), {pos.x, pos.y, pos.z, n.x, n.y, n.z, uv.x, uv.y, t.x, t.y, t.z});
}

// Triangle-based Tangent Calculation
static glm::vec3 CalcTangent(glm::vec3 p1, glm::vec3 p2, glm::vec3 p3, glm::vec2 uv1, glm::vec2 uv2, glm::vec2 uv3)
{
...
std::shared_ptr<Mesh> GeometryFactory::CreateSkyboxCube(float size)
{
    ...
    float h = size * 0.5f;
    static const float skyboxVertices[] = {-1.0f, 1.0f,  -1.0f, -1.0f, -1.0f, -1.0f, 1.0f,  -1.0f, -1.0f,
...
std::shared_ptr<Mesh> GeometryFactory::CreateCylinder(float radius, float height, int segments)
{
    ...
    for (int i = 0; i < segments; ++i)
    {
        indices.insert(indices.end(), {bottomCenterIndex, bottomCenterIndex + i + 1, bottomCenterIndex + i + 2});
    }
    ...
    for (int i = 0; i < segments; ++i)
    {
        indices.insert(indices.end(), {topCenterIndex, topCenterIndex + i + 2, topCenterIndex + i + 1});
    }
...
std::shared_ptr<Mesh> GeometryFactory::CreateCone(float radius, float height, int segments)
{
    ...
    for (int i = 0; i < segments; ++i)
    {
        indices.insert(indices.end(), {baseCenterIndex, baseStart + i + 1, baseStart + i});
    }
...
std::shared_ptr<Mesh> GeometryFactory::CreateTorus(float radius, float tubeRadius, int segments, int rings)
{
    ...
    for (int ring = 0; ring <= rings; ++ring)
    {
        ...
        for (int seg = 0; seg <= segments; ++seg)
        {
            ...
            glm::vec3 n{cosU * cosV, sinV, sinU * cosV};
