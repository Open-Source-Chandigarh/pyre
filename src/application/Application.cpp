...
void Application::ConfigureInput()
{
    ...
    // camera Movement
    input->BindKeyContinuous(inputMapper.GetKey("MoveForward", GLFW_KEY_W),
                             [app](float dt) { app->camera.ProcessKeyboard(FORWARD, dt); });
    input->BindKeyContinuous(inputMapper.GetKey("MoveBackward", GLFW_KEY_S),
                             [app](float dt) { app->camera.ProcessKeyboard(BACKWARD, dt); });
    input->BindKeyContinuous(inputMapper.GetKey("MoveLeft", GLFW_KEY_A),
                             [app](float dt) { app->camera.ProcessKeyboard(LEFT, dt); });
    input->BindKeyContinuous(inputMapper.GetKey("MoveRight", GLFW_KEY_D),
                             [app](float dt) { app->camera.ProcessKeyboard(RIGHT, dt); });

    // mouse
    input->BindMouseMove([app](double x, double y) { app->camera.ProcessMouseMovement((float) x, (float) y); });
    input->BindScroll([app](double y) { app->camera.ProcessMouseScroll((float) y); });
    ...
    input->BindKeyEvent(GLFW_KEY_RIGHT, GLFW_RELEASE, [SwitchScene]() { SwitchScene(1); });
    input->BindKeyEvent(GLFW_KEY_LEFT, GLFW_RELEASE, [SwitchScene]() { SwitchScene(-1); });

    // camera reset
    input->BindKeyEvent(inputMapper.GetKey("ResetCamera", GLFW_KEY_R), GLFW_RELEASE, [app]() { app->camera.Reset(); });

    // debug options
    input->BindKeyEvent(inputMapper.GetKey("ToggleWireframe", GLFW_KEY_F), GLFW_RELEASE,
                        [app]()
                        {
