...
Space::Space()
    : planetShader(nullptr), asteroidShader(nullptr), rotationAngle(0.0f), rotationSpeed(50.0f), planet(nullptr),
      asteroid(nullptr)
{
}
...
void Space::Init(AppState &appState)
{
    ...
    std::vector<std::string> faces = {"resources/textures/space/right.png", "resources/textures/space/left.png",
    ...
    if (asteroid)
        asteroid->SetupInstancing(asteroidTransforms);
...
void Space::OnActivate(AppState &appState)
{
    ...
    appState.lightManager->SetDirectional(sunDir, glm::vec3(0.02f), glm::vec3(1.2f, 1.1f, 1.0f), glm::vec3(0.2f));
