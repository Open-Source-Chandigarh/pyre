...
void Test::Init(AppState &appState)
{
    ...
    auto floorDisplacementMap =
        ResourceManager::LoadTexture("resources/textures/bricks_disp.jpg", TextureType::TEX_DISPLACEMENT);
    ...
    for (int layer = 0; layer < base; layer++)
    {
        ...
        for (int i = 0; i < base - layer; i++)
        {
            ...
            {
                std::shared_ptr<Entity> e = Entity::Create("Crate_" + std::to_string(layer) + "_" + std::to_string(i) +
                                                           "_" + std::to_string(j));
...
void Test::OnActivate(AppState &appState)
{
    ...
    appState.lightManager->SetDirectional(glm::vec3(0.0f), glm::vec3(0.0f), glm::vec3(0.0f), glm::vec3(0.0f));
