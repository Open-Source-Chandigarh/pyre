...
class Framebuffer
{
  public:
    Framebuffer(unsigned int width, unsigned int height, bool withDepth = true, bool multiSampled = false,
                ...
                unsigned int numColorAttachments = 1);
    ~Framebuffer();
    ...
    GLuint GetIntermediateTexture(unsigned int index = 0) const
    {
        if (multiSampled)
        {
            if (index >= screenTextures.size())
                return 0;
            return screenTextures[index];
        }
        else
            return GetColorTexture(index);
    }
  ...
  private:
    ...
    GLuint fbo = 0;
    std::vector<unsigned int> colorTextures; // could be MSAA or standard (can have more than 1 color attachments like
                                             // one for color and one for brightness)
